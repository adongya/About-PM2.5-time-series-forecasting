def VMD_ADD(signal, K):
    ...
    # print(f_mirror)
    f_mirror[0:int(T/2)]=signal[int(-(T/2)-1)::-1]
    # print(f_mirror)
    f_mirror[int(T/2):int(3*T/2)]= signal
    # print(f_mirror)
    f_mirror[int(3*T/2):int(2*T)]=signal[-1:int(-T/2-1):-1]
    # print(f_mirror)
    f=f_mirror
    # print('f_mirror')
    # print(f_mirror)
    ...
    #     print('-----')
        ...
        if DC==False:
            omega_plus[n, k-1]=np.dot(freqs[int(T/2):T],np.square(np.abs(u_hat_plus[n,int(T/2):T,k-1])).T)/np.sum(np.square(np.abs(u_hat_plus[n,int(T/2):T,k-1])))
    ...
    u_hat[int(T/2):T,:]= np.squeeze(u_hat_plus[N-1,int(T/2):T,:])
    # print('u_hat')
    # print(u_hat.shape)
    # print(u_hat)
    u_hat[int(T/2):0:-1,:]=np.squeeze(np.conj(u_hat_plus[N-1,int(T/2):T,:]))
    ...
    # remove mirror part 
    u=u[:,int(T/4):int(3*T/4)]

    # print(u_hat.shape)
    #recompute spectrum
    u_hat = np.zeros((int(T/2),K),dtype=complex)
    ...
    u_new[5] = u[6]
    u_new[6] = u[7]
    # plt.subplot(9, 1, 1), plt.plot(signal[:1000], 'k'), plt.legend(loc=1,labels=['orignal'],fontsize=12)
    # plt.subplot(9, 1, 2), plt.plot(u[0][:1000],),plt.ylabel("IMF1")
    # plt.subplot(9, 1, 3), plt.plot(u[1][:1000]),plt.ylabel("IMF2")
    # plt.subplot(9, 1, 4), plt.plot(u[2][:1000]),plt.ylabel("IMF3")
    # plt.subplot(9, 1, 5), plt.plot(u[3][:1000]),plt.ylabel("IMF4")
    # plt.subplot(9, 1, 6), plt.plot(u[4][:1000]),plt.ylabel("IMF5")
    # plt.subplot(9, 1, 7), plt.plot(u[5][:1000]),plt.ylabel("IMF6")
    # plt.subplot(9, 1, 8), plt.plot(u[6][:1000]),plt.ylabel("IMF7")
    # plt.subplot(9, 1, 9), plt.plot(u[7][:1000]),plt.ylabel("IMF8")
    # plt.subplot(9, 1, 10), plt.plot(u[8]),plt.legend(loc=1,labels=['u8'],fontsize=12)
    # plt.show()
    # plt.plot(signal)
    # plt.show()
    # plt.plot(u[0])
